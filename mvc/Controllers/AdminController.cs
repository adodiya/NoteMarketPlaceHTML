    ...
    public class AdminController : Controller
    {
        ...
        public ActionResult DownloadedNotes(string searchstring, string dropdownsearchNote, string dropdownsearchSeller, string dropdownsearchBuyer, string sortby, int? page)
        {
            ...
            ViewBag.SortByBuyer = string.IsNullOrEmpty(sortby) ? "Buyer desc" : "";
                ...
                ViewBag.dropdownsearchNote = new SelectList(db.Notes.Where(model => model.Status == 4).ToList(), "Title", "Title");
                ViewBag.dropdownsearchSeller = new SelectList(db.Users.Where(model => model.RoleID == 1).ToList(), "FirstName", "FirstName");
                ViewBag.dropdownsearchBuyer = new SelectList(db.Users.Where(model => model.RoleID == 1).ToList(), "FirstName", "FirstName");
                //var downloads = db.Downloads.AsQueryable();
                var downloads = db.Downloads.Where(model => model.IsAttachmentDownloaded == true);

            if (!string.IsNullOrEmpty(searchstring))
            {
                downloads = db.Downloads.Where(model => model.NoteTitle.Contains(searchstring) || model.NoteCategory.Contains(searchstring) || model.User.FirstName.Contains(searchstring) || model.User.LastName.Contains(searchstring) || model.User1.FirstName.Contains(searchstring) || model.User1.LastName.Contains(searchstring));
            }
...
//var notes = db.Notes.AsQueryable();
                ...
                if (!string.IsNullOrEmpty(dropdownsearch))
                {
                    notes = db.Notes.Where(model => model.User.FirstName.Contains(dropdownsearch));
        ...
        public ActionResult Members(string searchstring, string sortby, int? page)
        {
           ...
           var downloads = db.Users.Where(model => model.RoleID.Equals(1));
                ViewBag.FirstName = string.IsNullOrEmpty(sortby) ? "Title desc" : "";
                ViewBag.LastName = string.IsNullOrEmpty(sortby) ? "Category desc" : "";
                ViewBag.SortbyDate = string.IsNullOrEmpty(sortby) ? "Date desc" : "";
                if (!string.IsNullOrEmpty(searchstring))
                {
                    downloads = downloads.Where(model => model.FirstName.Contains(searchstring) || model.LastName.Contains(searchstring));
                }
                switch (sortby)
            {
                case "Title desc":
        ...
        {
            if (Session["UserID"] != null && (int)Session["Role"] == 3)
            {
                if (id == null)
                {
                    User_UserProfile obj = new User_UserProfile();
