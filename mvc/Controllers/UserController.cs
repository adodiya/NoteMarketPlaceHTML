...
namespace NotesMarketPlace.Controllers
{
    public class UserController : Controller
    {
           ...
           if(Session!=null)
            ...
            if (!string.IsNullOrEmpty(searchString2))
            {
                ...
                switch (sortby2)
                {
                    ...
                    default:
                        note.Inprogressnotes = db.Notes.Where(model => model.SellerID.Equals(id) && model.Status != 4).ToPagedList(page2 ?? 1, 5);
                        break;
                }
                return View(note);
            }
        ...
        public ActionResult Dashboard2(int? page1,string searchString1, string sortby1)

        {
            int id = (int)Session["UserID"];
            var note = db.Notes.Where(model => model.Status == 4);
            ViewBag.Count = db.Downloads.Where(model => model.Seller.Equals(id) && model.IsAttachmentDownloaded.Equals(true)).Select(model => model.NoteID).Distinct().Count();
            ViewBag.Sum = db.Downloads.Where(model => model.Seller.Equals(id) && model.DownloadAllowed.Equals(true) && model.IsPaid.Equals(true)).Select(model => model.Price).Sum();
            ...
            return View(note.ToPagedList(page1??1,5));
        }
